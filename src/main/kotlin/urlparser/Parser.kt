...
import urlparser.ParsingResult.NoValue
import urlparser.ParsingResult.ValueExtracted
import java.util.*
...
inline fun <reified T : Any> parse(url: String,
                                   noinline contextProvider: (KClass<*>)-> Set<KClass<out Any>>): Optional<T> {
    val parts = url.trim('/').split('/').filter { it.isNotEmpty() }

    return Optional.ofNullable(buildEntity(parts, T::class, contextProvider) { kClass, entityName ->
        ...
        val nameInPlural = "${name}s".takeIf { name != null }
        name == realEntityName || nameInPlural == realEntityName
    })
}
